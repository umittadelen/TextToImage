...
const promptsFile = window.location.pathname.includes('/hidden') 
    ? './json/prompts-hidden.json' 
    : './json/examplePrompts.json';

// Load and populate both selects
loadJsonAndPopulateSelect('./json/models.json', 'model', populateModels);
loadJsonAndPopulateSelect(promptsFile, 'example_prompt', populateExamplePrompts);
loadJsonAndPopulateSelect('./json/dimensions.json', 'example_size', populateExampleSizes);
